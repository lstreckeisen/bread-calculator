...
export function initPdfExport(section) {
    const pdfBtn = section.querySelector('#pdf-btn');
...
function exportPdf() {
    ...
    function tableRow(label, value, indent = 0, bold = false) {
        ensureSpace(LINE_H);
    ...
    if (result.autolyse) {
        ...
        if (result.autolyse.bassinageWater > 0)
            tableRow('Bassinage', formatG(result.autolyse.bassinageWater), SUB_INDENT);
    ...
    if (lv.levainTotal > 0)
        tableRow('Levain', formatG(lv.levainTotal));
    if (lv.yeastGrams > 0)
        tableRow(yeastLabel, formatG(lv.yeastGrams));
    if (result.enrichments?.honeyGrams)
        tableRow('Honig', formatG(result.enrichments.honeyGrams));
    if (result.enrichments?.maltGrams)
        tableRow('Backmalz', formatG(result.enrichments.maltGrams));
    ...
    if (result.doughSplit) {
        ...
        if (result.doughSplit.trimLoss > 0)
            tableRow('Teigverlust', formatG(result.doughSplit.trimLoss), SUB_INDENT);
        y += 4;
    }
    // ─── Gärzeiten ───────────────────────────────────────────────────────────
