...
import type { InclusionEntry, InclusionsConfig } from '../types'

function renderRow(entry: InclusionEntry, container: HTMLElement): HTMLElement {
...
function readInclusions(container: HTMLElement): InclusionEntry[] {
  return Array.from(container.querySelectorAll<HTMLElement>('.inclusion-row')).map(row => ({
    name: row.querySelector<HTMLInputElement>('.inclusion-name')!.value.trim(),
    amountG: parseFloat(row.querySelector<HTMLInputElement>('.inclusion-amount')!.value) || 0,
    absorptionCompensation: row.querySelector<HTMLInputElement>('.inclusion-abs-check')!.checked,
    absorptionPct: parseFloat(row.querySelector<HTMLInputElement>('.inclusion-abs-pct')!.value) || 30,
  }))
}

function updateInclusions(container: HTMLElement) {
  const toggle = document.getElementById('inclusions-toggle') as HTMLInputElement | null
  const cfg: InclusionsConfig = {
...
export function initInclusions(section: HTMLElement): void {
  const toggle = section.querySelector<HTMLInputElement>('#inclusions-toggle')!
  const panel = section.querySelector<HTMLElement>('#inclusions-panel')!
  const container = section.querySelector<HTMLElement>('#inclusions-rows')!
  const addBtn = section.querySelector<HTMLButtonElement>('#add-inclusion-btn')!
