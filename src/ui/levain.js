...
function buildFlourSelects(grainId, typeId, defaultGrain, defaultType) {
...
export function initLevain(section) {
    const modeATrigger = section.querySelector('#levain-mode-a');
    const modeBTrigger = section.querySelector('#levain-mode-b');
    const panelA = section.querySelector('#levain-panel-a');
    const panelB = section.querySelector('#levain-panel-b');
    // Mode A inputs
    const starterHydA = section.querySelector('#starter-hydration-a');
    const levainPctA = section.querySelector('#levain-pct-a');
    // Mode B inputs
    const starterHydB = section.querySelector('#starter-hydration-b');
    const anstellgut = section.querySelector('#anstellgut-grams');
    const levainFlour = section.querySelector('#levain-flour-grams');
    const levainHydration = section.querySelector('#levain-hydration');
    const hydQuick50 = section.querySelector('#hyd-quick-50');
    const hydQuick100 = section.querySelector('#hyd-quick-100');
    ...
    const { grainSel: grainSelA, typeSel: typeSelA } = buildFlourSelects('levain-flour-grain-a', 'levain-flour-type-a', initGrain, initType);
    const { grainSel: grainSelB, typeSel: typeSelB } = buildFlourSelects('levain-flour-grain-b', 'levain-flour-type-b', initGrain, initType);
    ...
    function buildCfgA() {
        return {
            ...
            flourGrain: grainSelA.value,
            flourType: typeSelA.value,
        };
    }
    function buildCfgB() {
        return {
            ...
            flourGrain: grainSelB.value,
            flourType: typeSelB.value,
        };
    }
