...
function validate(input) {
    ...
    if (input.preparations.kochstueck.enabled) {
        ...
        if (ks.flourGrams <= 0)
            errors.push('Kochstück-Mehlmenge muss grösser als 0 sein.');
        if (ks.waterGrams <= 0)
            errors.push('Kochstück-Wassermenge muss grösser als 0 sein.');
    }
    ...
    if (mode === 'sourdough-direct') {
        ...
        if (input.levain.yeastGrams <= 0)
            errors.push('Hefemenge muss grösser als 0 sein.');
    }
    ...
    const sourdoughConfig = mode === 'sourdough-direct' || mode === 'sourdough-build'
        ...
        : (mode === 'hybrid-yeast-primary' || mode === 'hybrid-sourdough-primary')
            ...
            : null;
    if (sourdoughConfig) {
        const levainFlourInBlend = input.flourBlend.some(e => e.grain === sourdoughConfig.flourGrain && e.type === sourdoughConfig.flourType);
...
export function calculate(input) {
    ...
    const perFlour = input.flourBlend.map(entry => {
        ...
        let deduction = 0;
        if (isLevainFlour)
            deduction += levainResult.flourInLevain;
        if (isKsFlour)
            deduction += kochstueckFlourDeduction;
