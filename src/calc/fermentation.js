 ...
 */
export function estimateTime(levainPct, temp, refTimeHours) {
 ...
 */
export function estimateLevainPct(targetHours, temp, refTimeHours) {
    const tempFactor = Math.pow(Q10, (REF_TEMP - temp) / 10);
    return REF_LEVAIN_PCT * refTimeHours * tempFactor / targetHours;
}
export function calcStuckgare(cfg, levainPct, pieceWeightG) {
...
export function calcFermentation(cfg, levainPct, pieceWeightG = null) {
