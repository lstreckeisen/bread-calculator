import type { DoughSplitConfig, DoughSplitResult } from '../types'

export function calcDoughSplit(cfg: DoughSplitConfig, totalDoughWeight: number): DoughSplitResult {
  const n = cfg.numPieces

  if (cfg.mode === 'equal') {
    const pieceWeight = totalDoughWeight / n
    const pieces = Array(n).fill(pieceWeight) as number[]
    return {
      totalDoughWeight,
      pieces,
      trimLoss: 0,
