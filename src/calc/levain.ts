...
export function convertLevainToFreshYeast(levainPct: number, totalFlour: number): number {
  const levainGrams = totalFlour * levainPct / 100
  return Math.round(levainGrams * LEVAIN_TO_FRESH_YEAST_RATIO * 10) / 10
}

export function convertFreshToDryYeast(freshGrams: number): number {
  return Math.round(freshGrams * FRESH_TO_DRY_YEAST_RATIO * 10) / 10
}

export function convertDryToFreshYeast(dryGrams: number): number {
  return Math.round(dryGrams / FRESH_TO_DRY_YEAST_RATIO * 10) / 10
}

export function convertFreshYeastToLevainPct(freshGrams: number, totalFlour: number): number {
  if (totalFlour <= 0) return 20
...
export function calcSourdoughDirect(cfg: SourdoughDirectConfig, totalFlour: number): LevainResult {
...
export function calcSourdoughBuild(cfg: SourdoughBuildConfig): LevainResult {
...
export function calcFreshYeast(cfg: FreshYeastConfig): LevainResult {
  return {
    levainTotal: 0,
    flourInLevain: 0,
    waterInLevain: 0,
...
export function calcDryYeast(cfg: DryYeastConfig): LevainResult {
  return {
    levainTotal: 0,
    flourInLevain: 0,
    waterInLevain: 0,
...
export function calcHybridYeastPrimary(cfg: HybridYeastPrimaryConfig, totalFlour: number): LevainResult {
...
export function calcHybridSourdoughPrimary(cfg: HybridSourdoughPrimaryConfig, totalFlour: number): LevainResult {
  const levainResult = cfg.levain.mode === 'sourdough-direct'
    ? calcSourdoughDirect(cfg.levain, totalFlour)
    : calcSourdoughBuild(cfg.levain)
  const yeastResult = cfg.yeast.mode === 'fresh-yeast'
...
export function calcLeaven(cfg: LeavenConfig, totalFlour: number): LevainResult {
  switch (cfg.mode) {
